...
interface PriceData {
  date: Date; // Date will be in YYYY-MM-DD string format
  price: number;
}

interface BrickPriceChartProps {
    selectedCity: string ;
}

const brickChartConfig = {
    price: {
      label: "Price",
      color: "red",
    },
  ...
  export default function BrickPricehart({ selectedCity }: BrickPriceChartProps) {
    ...
    React.useEffect(() => {
        async function fetchBrickData() {
            try {
                ...
                if (error) {
                    console.error('Error fetching brick prices:', error);
                    return;
                }
    ...
    const latestBrickPrice = React.useMemo(() => {
      ...
      <Card>
        <CardHeader className="flex items-center justify-between space-y-0 border-b p-0 sm:flex-row">
          <div className="flex flex-1 flex-col lg:text-4xl justify-center gap-1 px-6 py-5 sm:py-6">
            <CardTitle>Bricks Price Chart - {selectedCity.charAt(0).toUpperCase() + selectedCity.slice(1)}</CardTitle>
          </div>
          <div className="flex items-center px-6 py-4 sm:px-8 sm:py-6">
           <div className="flex flex-col items-center gap-1 text-center">
            ...
            <span className="text-xl font-bold leading-none lg:text-3xl sm:text-2xl">
              ₹{latestBrickPrice.toLocaleString()}/pc
            </span>
          </div>
         </div>
        </CardHeader>
          ...
          >
            ...
            >
              ...
              <Line
                dataKey="price"
                type="monotone"
                stroke={brickChartConfig?.price?.color}
                strokeWidth={3}
                dot={true}
                name="Price"
              />
